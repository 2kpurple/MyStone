package org.purplek.hearthstone;

public class Constant {
    public static final int ADD_DATA = 101;
    public static final int NEW_QUERY = 102;
    public static final int QUERY_FINISH = 103;
    public static final int SEARCH_FINISH = 104;
    public static final int DELETE_SUCCESS = 105;
    public static final int CARDS_NOT_ENOUGH = 106;
    public static final int INSERT_SUCCESS = 107;
    public static final int UPDATE_SUCCESS = 108;
    
    public static final String CLASS_KEY = "class";
    public static final String COLL_ID = "coll_id";
    public static final String COLL_NAME = "coll_name";
    public static final String CARD_DETIAL_MODE = "mode";
    public static final String LIST = "list";
    public static final String NUM = "num";
    public static final String COLLECTION = "collection";
    
    public static final int DETAIL_FINISH = 201;
    public static final int EDIT_FINISH = 202;
    
    public static final String[] COST_ARRAY = {"0", "1", "2", "3", "4", "5", "6", "7+"};
}
